<template>
    <el-config-provider :locale="locales.find(x => {return x.name === $i18n.locale})">
        ...
        <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
            <el-menu
                default-active="2"
                class="w-72 h-full"
                @open="handleOpen"
                ...
                @select="handleSelect"
            >
                <div class="flex w-full h-16 shrink-0 items-center">
                    <Link :href="route('dashboard')">
                        <application-mark class="w-48 mx-auto" />
                    </Link>
                </div>
